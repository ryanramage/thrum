...
function chordProgression(chords, patternStrOrOptions, options = {}) {
  ...
  const ticksPerBar = 96 // 4 beats * 24 ticks per beat
  
  // If pattern is provided, use it with smart triggering
  if (patternStr) {
    return pattern(patternStr).play((state) => {
      // Use absoluteTick to calculate which chord we should be on
      const absoluteBar = Math.floor(state.absoluteTick / ticksPerBar)
      const chordIndex = Math.floor(absoluteBar / barsPerChord) % chords.length
      const barInChordSection = absoluteBar % barsPerChord
      
      // Only play on the first bar of each chord section
      if (barInChordSection === 0) {
        const chordActions = chord(chords[chordIndex], opts)(state)
        return { actions: chordActions }
      }
...
/**
 * Create a bassline with note sequence and rhythm pattern
 * @param {Array} notes - Array of MIDI note numbers or note sequences
 * @param {string} patternStr - Rhythm pattern (e.g., 'x-x---x-x-x-----')
 * @param {Object} options - Options (velocity, length, channel, accent, slide)
 * @returns {Function} Track function
 */
function bassline(notes, patternStr, options = {}) {
  const velocity = options.velocity !== undefined ? options.velocity : 100
  const length = options.length !== undefined ? options.length : 24
  const channel = options.channel !== undefined ? options.channel : 0
  const accentVelocity = options.accentVelocity !== undefined ? options.accentVelocity : velocity + 20
  const accentPattern = options.accentPattern || null // e.g., 'x-------x-------'
  const slideLength = options.slideLength !== undefined ? options.slideLength : 12
  
  let stepCount = 0
  
  return pattern(patternStr).play((state) => {
    const noteCount = notes.length
    const noteIndex = stepCount % noteCount
    const currentNote = notes[noteIndex]
    
    // Check if this step should be accented
    let noteVelocity = velocity
    if (accentPattern) {
      const accentPos = stepCount % accentPattern.length
      if (accentPattern[accentPos] === 'x') {
        noteVelocity = accentVelocity
      }
    }
    
    stepCount++
    
    // Handle slide (portamento) - play a short note before the main note
    if (options.slide && stepCount > 1) {
      const prevNote = notes[(noteIndex - 1 + noteCount) % noteCount]
      const slideNote = prevNote < currentNote ? prevNote + 1 : prevNote - 1
      
      return {
        actions: [
          note(slideNote, { velocity: noteVelocity - 20, length: slideLength, channel })(state),
          note(currentNote, { velocity: noteVelocity, length: length, channel })(state)
        ]
      }
    }
    
    return note(currentNote, { velocity: noteVelocity, length, channel })(state)
  })
}
...
/**
 * Helper to get section-relative tick for CC automation
 * When inside an arrangement section, use section-relative positioning
 */
function getSectionRelativeTick(state) {
  // Check if we're inside an arrangement section
  if (state.userState && state.userState.sectionBar !== undefined) {
    // Calculate tick relative to section start
    return (state.userState.sectionBar * 96) + (state.beat * 24) + state.tick
  }
  // Otherwise use absolute tick
  return state.absoluteTick
}
...
function ccRamp(controller, startValue, endValue, bars, options = {}) {
  ...
  return (state) => {
    ...
    // Use section-relative tick when inside arrangement
    const referenceTick = getSectionRelativeTick(state)
    const currentTick = loop ? (referenceTick % totalTicks) : Math.min(referenceTick, totalTicks - 1)
...
function ccLFO(controller, rate, depth, options = {}) {
  ...
  return (state) => {
    ...
    // Only send CC at resolution intervals
    if (referenceTick % resolution !== 0) {
      return { actions: []  }
    }
    
    const totalTicks = rate * 96
    const phase = (referenceTick % totalTicks) / totalTicks
    let modulation = 0
