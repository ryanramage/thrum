 ...
 * @param {string} patternStr - Pattern string for when chords play
 * @param {Object} options - Options (velocity, length, channel, spread)
 * @returns {Function} Track function
 */
function chordProgression(chords, patternStr, options = {}) {
  const barsPerChord = options.barsPerChord !== undefined ? options.barsPerChord : 1
  
  return pattern(patternStr).play((state) => {
    const chordIndex = Math.floor(state.bar / barsPerChord) % chords.length
    return chord(chords[chordIndex], options)(state)
  })
}
